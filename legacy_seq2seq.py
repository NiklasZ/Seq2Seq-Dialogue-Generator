# From Tensorflow 1.0, contrib/legacy_seq2seq/python/ops/seq2seq.py
...
def _extract_argmax_and_embed(embedding,
  ...
  def loop_function(prev, _):
    ...
    if p_r is not None:
        ...
        tf_p_r = tf.Variable(p_r, name='p_r', trainable=False, dtype=tf.float32)
        prev_symbol = math_ops.argmax(p*(tf.log(p)-tf.log(tf_p_r)), 1)
    else:
...
def embedding_attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    loop_function = _extract_argmax_and_embed(
        ...
        p_r=p_r) if feed_previous else None
    emb_inp = [
